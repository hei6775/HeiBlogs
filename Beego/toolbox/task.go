...
type Tasker interface {
	GetSpec() string   //get SpecStr
	GetStatus() string //get Errlist string
	Run() error        //Run all task
	SetNext(time.Time)
	GetNext() time.Time
	SetPrev(time.Time)
	GetPrev() time.Time
...
func getRange(expr string, r bounds) uint64 {
	...
	var (
		start, end, step uint
		rangeAndStep     = strings.Split(expr, "/")            // 每个n单位执行一次
		lowAndHigh       = strings.Split(rangeAndStep[0], "-") //时间范围内 仅支持两位
