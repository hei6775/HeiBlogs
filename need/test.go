...
//几何算法
func (this *_AstarPoint)CalcuHValE(end *_AstarPoint)(h int){
	this.hVal = int(math.Sqrt(float64((end.x-this.x)*(end.x-this.x)+(end.y-this.y)*(end.y-this.y))))
	return this.hVal
}

//对角算法
func (this *_AstarPoint)CalcuHvalD(end *_AstarPoint)(h int){
...
func (this *_AstarPoint) calcFVal(end *_AstarPoint) int {
	this.fVal = this.calcGVal() + this.CalcuHvalD(end)
	return this.fVal
}
...
type SearchRoad struct {
	...
	closeLi map[string]*_AstarPoint //closed表
	openLi  OpenList
	openSet map[string]*_AstarPoint //open表
	TheRoad []*_AstarPoint //路径
}
...
func NewSearchRoad(startx, starty, endx, endy int, m *Map) *SearchRoad {
	...
	heap.Init(&sr.openLi)
	//fmt.Println("sr.openLi:",sr.openLi)
	//fmt.Println("sr.openLi:",len(sr.openLi))
	//fmt.Println("sr.openLi:",cap(sr.openLi))
...
func (this *SearchRoad) FindoutRoad() bool {
	...
	for len(this.openLi) > 0 {
		num++
		//fmt.Println("this.openSet",this.openSet)
		//fmt.Println("this.closeLi",this.closeLi)
		// 将节点从开放列表移到关闭列表当中。
...
func main() {
	...
	fmt.Println("花费时间：",time.Since(stratime))
}